...
_vp = os.path.join(
    ...
    "python",
    "atlasent",
    "evidence_bundle_verifier.py",
)
_spec = _ilu.spec_from_file_location("evb_verifier", _vp)
_m = _ilu.module_from_spec(_spec)
sys.modules["evb_verifier"] = _m
_spec.loader.exec_module(_m)
canonical_json, record_entry_hash, merkle_root_hex = (
    _m.canonical_json,
    _m.record_entry_hash,
    _m.merkle_root_hex,
)
...
key_set = {
    "issuing_keys": [{"key_id": KEY_ID, "alg": "Ed25519", "public_key_b64": pub_b64}]
}
...
def base_bundle():
    ...
    b = {
        ...
        "issued_by": {
            "org_id": "0190a1b2-aaaa-7000-8000-0000000000aa",
            "issuer_kind": "atlasent-api",
...
broken["records"][1]["prev_hash"] = (
    "ff" * 32
)  # breaks the link (sig re-signed so only chain fails)

...
class TestHrActions:
    ...
    def test_protect_hr_access_revoke_machine_executable(self) -> None:
        ...
        with patch("atlasent.verticals.hr_actions.protect", return_value=mock_permit) as mock_protect:
    ...
    def test_protect_hr_offboard_sets_correct_context(self) -> None:
        ...
        assert call_kwargs["context"]["hitl_escalation"]["assigned_to_role"] == "hr-approver"
        assert result is mock_permit
    ...
    def test_protect_hr_role_escalate_sets_critical_risk(self) -> None:
        ...
        with patch("atlasent.verticals.hr_actions.protect", return_value=MagicMock()) as mock_protect:
            protect_hr_role_escalate(
                employee_id="emp-999",
                authorized_by="hr-bot",
...
class TestModelGovernance:
    ...
    def test_protect_model_governance_promote_sets_escalation(self) -> None:
        ...
        mock_permit = MagicMock()
        with patch("atlasent.verticals.model_governance.protect", return_value=mock_permit) as mock_protect:
        ...
        assert call_kwargs["context"]["hitl_escalation"]["assigned_to_role"] == "ml-governance-board"
        assert call_kwargs["context"]["hitl_escalation"]["quorum_required"] == "simple_majority"
        assert result is mock_permit

    def test_protect_model_governance_retire(self) -> None:
        """ml.model.retire should also go through governance board."""
        with patch("atlasent.verticals.model_governance.protect", return_value=MagicMock()) as mock_protect:
            protect_model_governance(
                "ml.model.retire",
                model_id="old-bert",
...
class TestDataDelete:
    ...
    def test_protect_customer_data_delete_happy_path(self) -> None:
        ...
        mock_permit = MagicMock()
        with patch("atlasent.verticals.data_delete.protect", return_value=mock_permit) as mock_protect:
        ...
        assert call_kwargs["context"]["hitl_escalation"]["assigned_to_role"] == "compliance-officer"
        assert result is mock_permit

    def test_protect_customer_data_delete_all_bases(self) -> None:
        """All valid GDPR bases are accepted."""
        bases = ("erasure_request", "retention_expired", "consent_withdrawn", "controller_instruction")
        for basis in bases:
            with patch("atlasent.verticals.data_delete.protect", return_value=MagicMock()):
    ...
    def test_protect_customer_data_delete_72h_wait(self) -> None:
        ...
        with patch("atlasent.verticals.data_delete.protect", return_value=MagicMock()) as mock_protect:
            protect_customer_data_delete(
...
class TestContractActions:
    ...
    def test_protect_contract_execute_critical_irreversible(self) -> None:
        ...
        mock_permit = MagicMock()
        with patch("atlasent.verticals.contract_actions.protect", return_value=mock_permit) as mock_protect:
    ...
    def test_protect_contract_amend_high_partial(self) -> None:
        ...
        with patch("atlasent.verticals.contract_actions.protect", return_value=MagicMock()) as mock_protect:
            protect_contract_action(
                "contract.amend",
                contract_id="ctr-002",
                authorized_by="legal-agent",
    ...
    def test_protect_contract_action_simple_majority_for_critical(self) -> None:
        ...
        quorum = mock_protect.call_args.kwargs["context"]["hitl_escalation"]["quorum_required"]
...
class TestPricingActions:
    ...
    def test_pricing_rule_publish_below_threshold_machine_executable(self) -> None:
        ...
        with patch("atlasent.verticals.pricing_actions.protect", return_value=MagicMock()) as mock_protect:
            protect_pricing_rule(
                rule_id="rule-001",
    ...
    def test_pricing_rule_publish_above_threshold_escalates(self) -> None:
        """pricing.rule.publish with change_pct >= 5 should not be machine_executable."""
        with patch("atlasent.verticals.pricing_actions.protect", return_value=MagicMock()) as mock_protect:
            protect_pricing_rule(
                rule_id="rule-002",
                authorized_by="pricing-bot",

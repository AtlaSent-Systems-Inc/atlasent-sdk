...
ReplayVarianceKind = Literal[
    "NONE", "POLICY_DRIFT", "ENVELOPE_DRIFT",
    "ENGINE_DRIFT", "CHAIN_TAMPER", "BUNDLE_MISSING",
]


class ReplayResponse(BaseModel):
    """Result of :meth:`AtlaSentClient.replay` / :meth:`AsyncAtlaSentClient.replay`.

    The SDK never raises for any variance kind — callers switch on
    ``variance_kind``. Three-way parity: V1 wire evaluator, control-plane
    OPA bundle, SDK (ADR-015 §Replay, parity v2).
    """
    ...
    engine_version_kind: str | None = None
    accepts_replay: bool
    envelope_verification: str | None = None
    replayed_at: str
    rate_limit: RateLimitState | None = None

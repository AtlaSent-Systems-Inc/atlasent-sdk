...
FinancialActionType = str  # canonical set documented in TS; open-ended for extensibility
...
class FinancialActionClass:
    ...
    reversible: bool
    autonomous_ceiling: Optional[float]
    ceiling_currency: Optional[CurrencyCode]
    created_at: str
...
DEFAULT_RISK_TIER_THRESHOLDS: tuple[RiskTierThreshold, ...] = (
    RiskTierThreshold(tier="low",      lower_bound=0.0,        upper_bound=1_000.0,      reference_currency="USD"),
    RiskTierThreshold(tier="medium",   lower_bound=1_000.0,    upper_bound=50_000.0,     reference_currency="USD"),
    RiskTierThreshold(tier="high",     lower_bound=50_000.0,   upper_bound=1_000_000.0,  reference_currency="USD"),
    RiskTierThreshold(tier="critical", lower_bound=1_000_000.0, upper_bound=None,         reference_currency="USD"),
)
...
def within_autonomous_ceiling(action_value: float, ceiling: Optional[float]) -> bool:

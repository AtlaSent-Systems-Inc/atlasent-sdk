...
class TestAsyncScimUsers:
    async def test_list_users(self, client: AsyncAtlaSentClient):
        with patch.object(
            client._client, "request", new=AsyncMock(return_value=_mock_response(LIST_USERS_RESP))
        ) as mock_req:
    ...
    async def test_list_users_with_filter(self, client: AsyncAtlaSentClient):
        with patch.object(
            ...
            result = await client.async_scim_list_users(
                ORG_ID, filter='userName eq "alice@example.com"', start_index=1, count=10
            )
    ...
    async def test_create_user(self, client: AsyncAtlaSentClient):
        with patch.object(
            client._client, "request", new=AsyncMock(return_value=_mock_response(SAMPLE_USER, 201))
        ) as mock_req:
    ...
    async def test_create_user_preserves_existing_schemas(self, client: AsyncAtlaSentClient):
        user_with_schemas = {"schemas": ["custom-schema"], "userName": "bob@example.com"}
        with patch.object(
            client._client, "request", new=AsyncMock(return_value=_mock_response(SAMPLE_USER, 201))
        ) as mock_req:
    ...
    async def test_get_user(self, client: AsyncAtlaSentClient):
        with patch.object(
            client._client, "request", new=AsyncMock(return_value=_mock_response(SAMPLE_USER))
        ) as mock_req:
    ...
    async def test_patch_user(self, client: AsyncAtlaSentClient):
        ...
        with patch.object(
            ...
            result = await client.async_scim_patch_user(ORG_ID, "user-1", ops)
    ...
    async def test_delete_user(self, client: AsyncAtlaSentClient):
        with patch.object(
            client._client, "request", new=AsyncMock(return_value=_mock_response(None, 204))
        ) as mock_req:
...
class TestAsyncScimGroups:
    async def test_list_groups(self, client: AsyncAtlaSentClient):
        with patch.object(
            client._client, "request", new=AsyncMock(return_value=_mock_response(LIST_GROUPS_RESP))
        ) as mock_req:
    ...
    async def test_create_group(self, client: AsyncAtlaSentClient):
        with patch.object(
            client._client, "request", new=AsyncMock(return_value=_mock_response(SAMPLE_GROUP, 201))
        ) as mock_req:
            result = await client.async_scim_create_group(ORG_ID, {"displayName": "Engineering"})
    ...
    async def test_get_group(self, client: AsyncAtlaSentClient):
        with patch.object(
            client._client, "request", new=AsyncMock(return_value=_mock_response(SAMPLE_GROUP))
        ) as mock_req:
    ...
    async def test_replace_group(self, client: AsyncAtlaSentClient):
        with patch.object(
            ...
            result = await client.async_scim_replace_group(
...
class TestAsyncSiem:
    async def test_get_siem_config(self, client: AsyncAtlaSentClient):
        with patch.object(
            client._client, "request", new=AsyncMock(return_value=_mock_response(SIEM_CONFIG))
        ) as mock_req:
    ...
    async def test_upsert_siem_config(self, client: AsyncAtlaSentClient):
        ...
        assert f"/v1/orgs/{ORG_ID}/siem-config" in call_url
        sent_body = json.loads(mock_req.call_args[1]["content"])
        assert sent_body["destinationUrl"] == "https://splunk.example.com:8088/services/collector"
        assert sent_body["credential"] == "my-token"

    async def test_upsert_siem_config_validation_non_https(self, client: AsyncAtlaSentClient):
    ...
    async def test_upsert_siem_config_invalid_auth_type(self, client: AsyncAtlaSentClient):
    ...
    async def test_upsert_siem_config_batch_size_out_of_range(self, client: AsyncAtlaSentClient):
    ...
    async def test_upsert_siem_config_retry_count_out_of_range(self, client: AsyncAtlaSentClient):
    ...
    async def test_upsert_siem_no_credential_omitted_from_body(self, client: AsyncAtlaSentClient):
        with patch.object(
            client._client, "request", new=AsyncMock(return_value=_mock_response(SIEM_CONFIG))
        ) as mock_req:
            await client.async_upsert_siem_config(
...
class TestAsyncEvidenceExports:
    ...
    async def test_list_evidence_exports_invalid_regime(self, client: AsyncAtlaSentClient):
    ...
    async def test_create_evidence_export_invalid_regime(self, client: AsyncAtlaSentClient):
        with pytest.raises(ValueError, match="regime must be one of"):

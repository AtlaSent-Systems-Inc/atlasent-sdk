...
class TestListEvidenceExports:
    def test_list_returns_exports(self):
        ...
        with patch.object(client._client, "request", return_value=_mock_response(payload)) as mock_req:
    ...
    def test_list_500_raises_server_error(self):
        client = _client()
        with patch.object(client._client, "request", return_value=_mock_response({"error": "internal"}, 500)):
...
class TestGetEvidenceExport:
    def test_get_returns_record(self):
        client = _client()
        with patch.object(client._client, "request", return_value=_mock_response(EXPORT_RECORD)) as mock_req:
            result = get_evidence_export(client, ORG_ID, EXPORT_ID)
        assert result["id"] == EXPORT_ID
        assert result["regime"] == "soc2_type_ii"
        assert mock_req.call_args[0][0] == "GET"
        assert f"/v1/orgs/{ORG_ID}/evidence-exports/{EXPORT_ID}" in mock_req.call_args[0][1]
    ...
    def test_get_404_raises(self):
        ...
        with patch.object(
            client._client,
            "request",
            return_value=_mock_response({"error": f"Evidence export {EXPORT_ID} not found"}, 404),
        ):
            with pytest.raises(AtlaSentError) as exc_info:
...
class TestCreateEvidenceExport:
    def test_create_happy_path(self):
        client = _client()
        with patch.object(client._client, "request", return_value=_mock_response(CREATE_RESPONSE, 201)) as mock_req:
    ...
    def test_create_with_bundle_id(self):
        client = _client()
        with patch.object(client._client, "request", return_value=_mock_response(CREATE_RESPONSE, 201)) as mock_req:
            create_evidence_export(client, ORG_ID, regime="gdpr", bundle_id="my-bundle-id")
        sent = json.loads(mock_req.call_args[1]["content"])
    ...
    def test_create_with_evidence(self):
        ...
        with patch.object(client._client, "request", return_value=_mock_response(CREATE_RESPONSE, 201)) as mock_req:
            create_evidence_export(client, ORG_ID, regime="soc2_type_ii", evidence=evidence)
        sent = json.loads(mock_req.call_args[1]["content"])
    ...
    def test_create_all_valid_regimes(self):
        ...
        for regime in ("soc2_type_ii", "hipaa", "gdpr"):
            with patch.object(client._client, "request", return_value=_mock_response(CREATE_RESPONSE, 201)):
    ...
    def test_create_400_raises(self):
        ...
        with patch.object(
            client._client,
            "request",
            return_value=_mock_response({"error": "window.from must be earlier than window.to", "field": "window"}, 400),
        ):
            with pytest.raises(AtlaSentError) as exc_info:
                create_evidence_export(
                    client,
                    ORG_ID,
                    regime="soc2_type_ii",
                    window={"from": "2026-05-01T00:00:00Z", "to": "2026-02-01T00:00:00Z"},
                )

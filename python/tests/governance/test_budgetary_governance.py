...
def test_hard_limit_exceeded() -> None:
    result = check_budget_constraints(
        ...
        current_monthly_spend=0,
        applicable_limits=(
            (_limit(limit_amount=100_000, enforcement="hard"), _state(50_000)),
        ),
...
def test_soft_limit_warns_but_permits() -> None:
    result = check_budget_constraints(
        ...
        current_monthly_spend=0,
        applicable_limits=(
            (_limit(limit_amount=100_000, enforcement="soft"), _state(50_000)),
        ),
        applicable_constraints=(_constraint(max_single_transaction=100_000),),
    )
    assert result.permitted is True
...
def test_anonymous_agent_blocked() -> None:
    ...
    assert result.permitted is False
    assert any(
        v.violation_type == "anonymous_agent_blocked" for v in result.hard_blocks
    )
...
def test_single_transaction_exceeds_blocks() -> None:
    ...
    assert result.permitted is False
    assert any(
        v.violation_type == "single_transaction_exceeds" for v in result.hard_blocks
    )
...
def test_daily_aggregate_exceeds_blocks() -> None:
    ...
    assert result.permitted is False
    assert any(
        v.violation_type == "daily_aggregate_exceeds" for v in result.hard_blocks
    )
...
def test_monthly_aggregate_exceeds_blocks() -> None:
    ...
    assert result.permitted is False
    assert any(
        v.violation_type == "monthly_aggregate_exceeds" for v in result.hard_blocks
    )


def test_action_type_filter_skips_non_matching_constraints() -> None:
    # Constraint targets refunds; action is a wire transfer,
    # so this constraint should not apply.
    result = check_budget_constraints(
        action_value=75_000,
        currency="USD",
        action_type="wire_transfer",
